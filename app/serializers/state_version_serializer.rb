...
class StateVersionSerializer < ApplicationSerializer
  ...
  attribute :hosted_state_download_url do |object|
    EncryptedStorage.storage_url({ id: object.id, class: object.class.name, filename: 'state' })
  end
  ...
  attribute :hosted_state_upload_url do |object|
    if object.state_file.present?
      ...
      EncryptedStorage.upload_url({ id: object.id, class: object.class.name, filename: 'state' })
    end
  end

  attribute :hosted_json_state_download_url do |object|
    EncryptedStorage.storage_url({ id: object.id, class: object.class.name, filename: 'state.json' })
  end
  ...
  attribute :hosted_json_state_upload_url do |object|
    if object.state_json_file.present?
      ...
      EncryptedStorage.upload_url({ id: object.id, class: object.class.name, filename: 'state.json' })
    end
  end
